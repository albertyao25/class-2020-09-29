...
d_2019 <-
  ...
  select('Course ID', 'Course Title', 'Course Name', 'Course Department', 'UGrad') %>%
  clean_names() %>%
  rename_with(~str_replace(.x, "course_", "")) %>%
  group_by(id, title, name, department) %>%
  summarise(u_grad = sum(u_grad)) %>%
  filter(u_grad >= 10)
...
read_enrollment <- function(file, misc_rows) {
  
  read_excel(file, skip = misc_rows + 1) %>%
  select("Course ID", "Course Title", "Course Name", "Course Department", "UGrad") %>%
  clean_names() %>%
  rename_with(~str_replace(.x, "course_", "")) %>%
  group_by(id, title, name, department) %>%
...
d_2019 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_12.20.19.xlsx", 2)
d_2018 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_10.24.18.xlsx", 1)
d_2017 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_10.20.17.xlsx", 2)
d_2016 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_fall_2016.xlsx", -1)
d_2015 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_fall_2015.xlsx", 2)
```
### Scene 4
...
enrollment <- bind_rows(list(d_2019, d_2018, d_2017, d_2016, d_2015), .id = "a") 
